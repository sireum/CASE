#include "tb_AssignmentCoordinatorTask.h"
#include <string.h>
#include <camkes.h>
...
bool tb_AssignmentCoordination_out_enqueue(const UXTASK__AssignmentCoordination_i * tb_AssignmentCoordination_out){
  bool tb_result = true;
  tb_result &= tb_AssignmentCoordination_out0_enqueue((UXTASK__AssignmentCoordination_i *) tb_AssignmentCoordination_out);
  return tb_result;
}
bool tb_TaskAutomationRequest_out_enqueue(const UXTASK__TaskAutomationRequest_i * tb_TaskAutomationRequest_out){
  bool tb_result = true;
  tb_result &= tb_TaskAutomationRequest_out0_enqueue((UXTASK__TaskAutomationRequest_i *) tb_TaskAutomationRequest_out);
  return tb_result;
}
