 ...
 * two modifications:
 * 1) The line in "src/tb_Decrypt_uart2self_Monitor.c" defining TB_VERIFY
 *    must be uncommented.
 * 2) The AUTOCORRES_HOME text below must be replaced by the path to the 
 *    AutoCorres package for Isabelle.
 *)
theory tb_Decrypt_uart2self_Monitor
...
install_C_file "src/tb_Decrypt_uart2self_Monitor.c"
autocorres[ts_rules = nondet, heap_abs_syntax] "src/tb_Decrypt_uart2self_Monitor.c"

context tb_Decrypt_uart2self_Monitor begin
...
definition is_queue :: "lifted_globals \<Rightarrow> bool" where
  "is_queue s \<equiv> front_'' s < 100 \<and> length_'' s \<le> 100"
...
lemma is_full_wp [wp]:
"\<lbrace> \<lambda>s. if queue_length (the_queue s) = 100 then Q 1 s else Q 0 s \<rbrace>
  is_full'
...
lemma enqueue_full:
  "\<lbrace> \<lambda>s. is_queue s \<and> queue_length (the_queue s) = 100 \<and> P s\<rbrace>
   mon_enqueue' x
...
apply (unfold mon_dequeue'_def)
  apply wp
  apply (auto simp: fun_upd_def is_queue_def the_queue_def tb_Decrypt_uart2self_Monitor.update_SMACCM_DATA__UART_Packet_i_def
