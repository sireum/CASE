 ...
 * two modifications:
 * 1) The line in "src/tb_Server_framing2self_Monitor.c" defining TB_VERIFY
 *    must be uncommented.
 * 2) The AUTOCORRES_HOME text below must be replaced by the path to the 
 *    AutoCorres package for Isabelle.
 *)
theory tb_Server_framing2self_Monitor
...
install_C_file "src/tb_Server_framing2self_Monitor.c"
autocorres[ts_rules = nondet, heap_abs_syntax] "src/tb_Server_framing2self_Monitor.c"

context tb_Server_framing2self_Monitor begin
...
apply (unfold mon_dequeue'_def)
  apply wp
  apply (auto simp: fun_upd_def is_queue_def the_queue_def tb_Server_framing2self_Monitor.update_tb_SMACCM_DATA__GIDL_container_def
