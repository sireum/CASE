 ...
 * two modifications:
 * 1) The line in "src/tb_UART_Driver_encrypt2self_Monitor.c" defining TB_VERIFY
 *    must be uncommented.
 * 2) The AUTOCORRES_HOME text below must be replaced by the path to the 
 *    AutoCorres package for Isabelle.
 *)
theory tb_UART_Driver_encrypt2self_Monitor
...
install_C_file "src/tb_UART_Driver_encrypt2self_Monitor.c"
autocorres[ts_rules = nondet, heap_abs_syntax] "src/tb_UART_Driver_encrypt2self_Monitor.c"

context tb_UART_Driver_encrypt2self_Monitor begin
...
definition is_queue :: "lifted_globals \<Rightarrow> bool" where
  "is_queue s \<equiv> front_'' s < 5 \<and> length_'' s \<le> 5"
...
lemma is_full_wp [wp]:
"\<lbrace> \<lambda>s. if queue_length (the_queue s) = 5 then Q 1 s else Q 0 s \<rbrace>
  is_full'
...
lemma enqueue_full:
  "\<lbrace> \<lambda>s. is_queue s \<and> queue_length (the_queue s) = 5 \<and> P s\<rbrace>
   mon_enqueue' x
...
lemma enqueue_not_full:
  "(\<And>s c. P (s\<lparr>contents_'' := c\<rparr>) = P s) \<Longrightarrow>
   ...
   \<lbrace> \<lambda>s. is_queue s \<and>
         q = the_queue s \<and>
         queue_length q < 5 \<and>
         is_valid_SMACCM_DATA__UART_Packet_i_C s x \<and>
         P s \<rbrace>
   mon_enqueue' x
   \<lbrace> \<lambda>r s. r = 1 \<and>
           the_queue s = enqueue q (heap_SMACCM_DATA__UART_Packet_i_C s x) \<and>
           is_queue s \<and>
           P s \<rbrace>!"
apply (unfold mon_enqueue'_def)
apply wp
...
lemma dequeue_not_empty:
  "(\<And>s f. P (s\<lparr>front_'' := f\<rparr>) = P s) \<Longrightarrow>
   ...
   \<lbrace> \<lambda>r s. r > 0 \<and>
           dequeue q = (heap_SMACCM_DATA__UART_Packet_i_C s x, the_queue s) \<and>
           is_queue s \<and>
           P s \<rbrace>!"
apply (unfold mon_dequeue'_def)
  apply wp
  apply (auto simp: fun_upd_def is_queue_def the_queue_def tb_UART_Driver_encrypt2self_Monitor.update_SMACCM_DATA__UART_Packet_i_def
